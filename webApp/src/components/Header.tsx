...
const Header = ({ title, arrowBack }: { title?: string, arrowBack?: any }) => {
	...
	return (
		<header className={pathname === "evenement" || "Evenement" ? "event" : ""}>
			<div className="title">
				<h1>{title}</h1>
			...
			<div className="links">
				...
				{session != null &&
					<NavLink to="" onClick={handleLogout} style={{ display: "flex", flexDirection: "column", alignItems: "center", cursor: "pointer" }}>
